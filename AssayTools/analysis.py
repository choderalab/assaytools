...
class CompetitiveBindingAnalysis(object):
    ...
    def _create_solutions_model(self, solutions):
        ...
        * parameter_names['concentrations'] : parameters associated with true concentrations of receptor and ligand solutions
        ...
        for solution in solutions.values():
            ...
            name = 'log concentration of %s' % solution.name
    ...
    def _create_dispensing_model(self):
        ...
        *
        ...
        self.parameter_names['well concentrations'] = dict()

        for well in self.wells:
            # Volumes dispensed into each well
            log_volumes = list()
            for component in well.properties['contents']:
                name = 'log volume of %s dispensed into well %s' % (component, well.humanize()) # TODO: Use plate name as well in case we may have well spanning multiple plates
            ...
            name = 'log volume of well %s' % well.humanize() # TODO: Use plate name too in case wells span multiple plates
            ...
            for component in well.properties['contents']:
                ...
                log_solution_concentration = self.model['log concentration of %s' % solution.name]
                log_solution_volume = model['log volume of %s dispensed into well %s' % (component, well.humanize())]
                log_total_volume = model['log volume of well %s' % well.humanize()]
    ...
    def _binding_reactions_for_well(self, well):
        ...
        all_species = self._all_initial_species_in_well(well)
        reactions = list()
        for reaction in self.reactions:
            # Decompose reaction into product and reactant sets
            (DeltaG, stoichiometry) = reaction
    ...
    def _conservation_equations_for_well(self, well):
        ...
        for conservation_equation in self.conservation_equations:
            ...
            if species in well.properties['contents']:
                name = 'log total concentration of %s in well %s' % (species, well.humanize())
                log_total_concentration = self.model[name]
    ...
    def _create_equilibrium_concentrations_model(self):
        ...
        for well in self.wells():
            ...
            name = 'log equilibrium concentration of all species in well %s' % (species, well.humanize())
            @pymc.deterministic(name=name)
            ...
            for species in all_equilibrium_species:
                ...
                @pymc.deterministic(name=name)
                def log_equilibrium_concentration(log_equilibrium_concentations=log_equilibrium_concentations):
                    return log_equilibrium_concentations[species]
                self.model[name] = log_equilibrium_concentration
                sel.parameter_names['well concentrations'].append(name)
    ...
    def _create_extinction_coefficients_model(self, inner_filter_effect):
        ...
        * fluorescence : True if fluorescence in use
        ...
        MAX_EXTINCTION_COEFFICIENT = Unit(100e3, '1/molar/centimeter') # maximum physically reasonable extinction coefficient
                ...
                for wavelength in measurements['absorbance'].keys():
                    ...
                    def absorbance_model(concentrations=concentrations, extinction_coefficients=extinction_coefficients, plate_absorbance=plate_absorbance):
                        ...
                        for species in concentrations.keys():
                            ec += extinction_coefficients[species] * path_length * concentrations[species]
    ...
    def _create_fluorescence_model(self, inner_filter_effect):
        ...
        for well in self.wells:
            log_well_volume = self.model['log volume of well %s' % well.humanize()]
            measurements = well.properties['measurements']
            if 'fluorescence' in measurements:
                for (excitation_wavelength, emission_wavelength, geometry) in measurements['fluorescence'].keys():
                    ...
                    def fluorescence_model(log_well_volume=log_well_volume,
                        concentrations=concentrations, quantum_yields=quantum_yields,
                        ...
                        # Compute path length
                        well_volume = np.exp(log_well_volume)
                        ...
                        for (concentration, excitation_extinction_coefficient, emission_extinction_coefficient) in zip(concentrations, excitation_extinction_coefficients, emission_extinction_coefficients):
                        ...
                        if geometry == 'top':
                            alpha = (ELC_excitation + ELC_emission)
                        ...
                        for (quantum_yield, concentration) in zip(quantum_yields, concentrations):
                    ...
                    self.model[name] = pymc.Normal(name, mu=fluorescence_model, tau=model['%s fluorescence precision' % geometry], observed=True, value=measured_fluorescence)
                    self.parameter_names['fluorescence'].append(name)
