...
def find_packages():
    """Find all of assaytools's python packages.
    Adapted from IPython's setupbase.py. Copyright IPython
    contributors, licensed under the BSD license.
    """
    packages = ['assaytools.scripts']
    for dir,subdirs,files in os.walk('AssayTools'):
        package = dir.replace(os.path.sep, '.')
        if '__init__.py' not in files:
            # not a package
            continue
        packages.append(package.replace('AssayTools', 'assaytools'))
    return packages
...
def write_version_py(VERSION, ISRELEASED, filename='AssayTools/version.py'):
    cnt = """
# THIS FILE IS GENERATED FROM ASSAYTOOLS SETUP.PY
